...
- 14 persist параметрів, 34 state keys
- **НЕ зупиняє обладнання** — protection.lockout зарезервовано (завжди false)
...
нормальному циклі (≥ min_compressor_run).
...
**Continuous Run (#8):**
Якщо компресор безперервно працює довше `max_continuous_run` хвилин — alarm.
Auto-clear при вимкненні компресора (навіть якщо manual_reset=true).

**Pulldown Failure (#9):**
Після запуску компресора фіксується початкова температура (evap_temp якщо доступна,
інакше air_temp). Якщо через `pulldown_timeout` хвилин температура не впала на
`pulldown_min_drop` градусів — alarm. Індикатор неефективної роботи системи.
...
## Таблиця state keys (34)

### Readonly (20)

| Key | Type | Опис |
|-----|------|------|
| protection.lockout | bool | Зарезервовано (завжди false) |
...
### Readwrite — параметри (15)

14 persist параметрів (таблиця вище) + `protection.reset_alarms` (write-only trigger) +
`protection.compressor_hours` (persist, мотогодини).
...
**Publish (19):** всі alarm + diagnostics state keys.
**Subscribe (16):** всі readwrite параметри + reset_alarms + compressor_hours.
...
- 2026-03-02 — Initial documentation. 10 monitors, CompressorTracker, RateTracker, 14 persist params.
