...
- **Level 2 (lockout):** після `max_continuous_retries` послідовних спрацювань — перманентна блокіровка всього обладнання. Потрібен ручний reset (`protection.reset_alarms`).
...
| protection.max_continuous_retries | 3 | 1 | 5 | 1 | разів | Макс. спроб перед lockout |
...
17 persist параметрів (14 в таблиці + `forced_off_min` + `max_continuous_retries` + `compressor_hours`) +
