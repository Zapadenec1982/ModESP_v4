...
> **СТАТУС: ЗАПЛАНОВАНО.** Ця функціональність ще не реалізована в коді.
...
> **СТАТУС: ЗАПЛАНОВАНО.** OneWire scan реалізовано через `GET /api/onewire/scan`, але manifest-driven `discovery` поле ще не підтримується генератором.
...
|------|--------|----------|
| `drivers/ds18b20/manifest.json` | ✅ Існує | MATCH_ROM multi-sensor, SEARCH_ROM scan |
| `drivers/relay/manifest.json` | ✅ Існує | GPIO relay з min on/off time |
| `drivers/ntc/manifest.json` | ✅ Існує | NTC через ADC (B-parameter) |
| `drivers/digital_input/manifest.json` | ✅ Існує | GPIO input з 50ms debounce |
| `drivers/pwm_fan/manifest.json` | ❌ Відсутній | Заплановано |
| `manifest_version` в board.json | ✅ Є | |
| `manifest_version` в bindings.json | ✅ Є | |
| `manifest_version` в project.json | ✅ Є | |
| Валідація в generate_ui.py | ✅ 19 валідаторів | V01-V19 |

### Нереалізовані секції стандарту

| Секція | Опис | Статус |
|--------|------|--------|
| 2.1 Driver UI | UI для налаштування драйверів (access_level, instance_per_binding) | Заплановано |
| 2.2 Discovery/Scan | Manifest-driven discovery (поле `discovery` в driver manifest) | Заплановано |
| 3.1.1 Role Context | Контекст ролі (expected_range, alarm defaults, color_zones) | Заплановано |
